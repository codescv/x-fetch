...
import json
from .scraper import fetch_posts, open_for_login, fetch_single_post
...
@app.command()
def post(
    url: str = typer.Argument(..., help="The URL of the X (Twitter) post to fetch"),
    ...
    """
    Fetch a single post and its comments from X.com.
    """
    ...
    if output_format == "text":
        typer.echo(f"Fetching post from {url}...")
    
    try:
        result = fetch_single_post(
            url=url,
            user_data_dir=user_data_dir,
            http_proxy=http_proxy,
            https_proxy=https_proxy,
        ...
        if not result:
            typer.secho("Failed to fetch post or no data returned.", fg=typer.colors.RED, err=True)
            raise typer.Exit(code=1)
        ...
        if output_format == "json":
            ...
            lines = []
            lines.append(f"\\n--- Post ---")
            lines.append(f"Author: {post_data.get('author_name')} ({post_data.get('author_handle')}) · {post_data.get('posted_at')}")
            lines.append(f"Text:\\n{post_data.get('text')}")
            if post_data.get('post_link'):
                lines.append(f"Link: {post_data.get('post_link')}")
            lines.append(f"Metrics: Comments: {post_data.get('comments')} | Retweets: {post_data.get('retweets')} | Likes: {post_data.get('likes')}")
            ...
            for i, c in enumerate(comments, start=1):
                lines.append(f"{i}. {c.get('author_name')} ({c.get('author_handle')}): {c.get('text')}")
                if c.get('post_link'):
                    lines.append(f"   Link: {c.get('post_link')}")
                lines.append("-" * 20)
        ...
        typer.secho(f"Error fetching post: {e}", fg=typer.colors.RED, err=True)
        raise typer.Exit(code=1)
